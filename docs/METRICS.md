...
**One chart** is the default. The CPU overview and main menu-bar popover can switch to **Logical processors**. Each processor has its own 0–100% history; the combined chart retains the full-machine scale described above. Blue is total utilization and pink is its system portion. Click a tile for an interactive history with the same selected time range. The grid automatically chooses rows, columns and chart sizes from the processor/thread count and available viewport. It shows every chart together when they fit; dense grids simplify labels while preserving full hover and inspection details. At a minimum legible size, larger collections scroll vertically with **Paged charts** available as an alternative. Filtering enlarges the remaining charts. The main window retains its chart mode, filter, pagination and expanded CPU details across adaptive width/height changes and metric switches. The system menu-bar monitor retains its own preferences while the app runs. Processor grid height responds to viewport height and processor count; very small viewports retain the legible scrolling fallback.
...
Registry properties are **driver-defined, not a documented cross-vendor telemetry contract**. Apple silicon and Intel Iris Plus were checked on physical hardware. Other Intel/AMD drivers may supply device utilization through the supported keys, but missing engine, memory or process counters remain unavailable. No private framework, administrator helper or SIP change is used. The automated multi-device and alternate-driver tests are fixtures, not physical multi-GPU/eGPU validation.
