...
struct ContentView: View {
  ...
  var subheading: String {
    ...
    case .ane: return "Estimated system power and visible direct-path connections."
  ...
  func overviewControls(_ layout: MonitorLayout) -> some View {
    HStack(spacing: 8) {
      if metric == .gpu {
        ...
        Text(metric == .ane ? "ANE activity" : metric.rawValue + " activity")
  ...
  var statusbar: some View {
    HStack(spacing: 10) {
      ...
      if metric == .ane {
        Text("·")
        Text("Estimated system ANE power · direct process connections")
          .foregroundStyle(theme.tertiary)
      }
