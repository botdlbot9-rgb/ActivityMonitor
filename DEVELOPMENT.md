...
- `ANECollector.swift`, `ANEEnergyReader.swift`, `ANEOverview.swift`: read-only Neural Engine driver descriptors, direct-path process connections, and optional system-wide estimated power from the macOS Energy Model. See [hardware validation](docs/ANE_VALIDATION.md).
