...
Live metrics use macOS APIs. The optional system ANE power estimate, controller Running-state time, and bandwidth-tier monitor samples use undocumented OS counters and may be unavailable; they cannot attribute activity, power, or inference time to a process. Bandwidth-tier events are not measured bytes per second. Some protected process counters are unavailable. Energy Impact, complete per-process Metal allocation inventories, and per-process packet counts are unavailable; driver-reported GPU usage and permitted per-process graphics ledgers are available, and the interface identifies those limitations. Sampling a protected process may be denied by macOS. Nothing is uploaded.
